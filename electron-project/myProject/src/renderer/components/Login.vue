  ...
  <div class="Login">
    <form>
      ...
      <div class="inputGroup inputGroup1">
        ...
        />
        <div class="warning">
          <span v-if="!errFlag">只能包含字符、数字和下划线</span>
          <span v-if="errFlag">您输入了无效的用户名,请重新输入</span>
        </div>
      </div>
      <div class="inputGroup inputGroup2">
        <label for="loginPassword" id="loginPasswordLabel">密码</label>
        <input type="password" id="loginPassword" v-model="pwValue" />
      ...
      <div class="inputGroup inputGroup3">
        <button
          id="login"
          @click="logIn"
          :class="errFlag ? 'disable' : ''"
          :disabled="errFlag ? true : false"
        >
          登录
        </button>
        <button
...
<script>
import { TweenMax, Power2, Expo, Quad } from 'gsap';
      ...
      this.$http
        ...
        .then(({ data }) => {
          if (data.code === '000000') {
            ...
            localStorage.setItem('loginName', this.nameValue);
...
.Login {
  width: 100%;
  height: 100%;
  background: url('../assets/imgs/cat.jpg');
    ...
    .inputGroup1 {
      ...
      &.focusWithText {
        .warning {
          ...
          transition: width 0.5s, height 0.5s, opacity 1s linear;
        }
      }
    }
    .inputGroup2 {
